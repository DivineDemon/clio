...
import { Clock, ExternalLink, Eye, Filter, GitBranch, GitFork, RefreshCw, Search, Star } from "lucide-react";
    ...
    repositories
      ...
      ?.sort((a, b) => {
        switch (filters.sortBy) {
          ...
          case "updated":
            return new Date(b.githubUpdatedAt).getTime() - new Date(a.githubUpdatedAt).getTime();
          case "created":
            return new Date(b.githubCreatedAt).getTime() - new Date(a.githubCreatedAt).getTime();
          default:
            return 0;
        }
      }) ?? [];
    ...
    return (
      <div className="rounded-lg bg-white shadow dark:bg-slate-800">
        <div className="border-gray-200 border-b px-6 py-4 dark:border-gray-700">
          <h2 className="font-semibold text-gray-900 text-lg dark:text-white">Your Repositories</h2>
        </div>
        <div className="p-6">
          <div className="py-12 text-center">
            <RefreshCw className="mx-auto h-8 w-8 animate-spin text-gray-400" />
            <p className="mt-4 text-gray-600 dark:text-gray-300">Checking GitHub App installation...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="rounded-lg bg-white shadow dark:bg-slate-800">
        ...
        <div className="p-6">
          <div className="py-12 text-center">
            ...
            <h3 className="mt-4 font-semibold text-gray-900 text-lg dark:text-white">GitHub App Not Installed</h3>
            <p className="mt-2 text-gray-600 dark:text-gray-300">
              Install the Clio GitHub App to access your repositories and start generating READMEs.
            </p>
            <div className="mt-6">
              <Button asChild>
                <a
        ...
        <div className="p-6">
          <div className="py-12 text-center">
            ...
            <h3 className="mt-4 font-semibold text-gray-900 text-lg dark:text-white">
              {installationStatus.source === "github" ? "GitHub App Detected!" : "Sync Required"}
            </h3>
            <p className="mt-2 text-gray-600 dark:text-gray-300">
            ...
            <div className="mt-6">
              <Button onClick={handleSyncInstallation} disabled={isSyncing} className="inline-flex items-center">
    ...
    <div className="rounded-lg bg-white shadow dark:bg-slate-800">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            {installationStatus?.installed && (
              <Button variant="outline" size="sm" onClick={handleSyncRepositories} disabled={isSyncing}>
                <RefreshCw className={`mr-2 h-4 w-4 ${isSyncing ? "animate-spin" : ""}`} />
                Re-sync Repos
              </Button>
            )}
      ...
      <div className="p-6">
        <div className="mb-6 flex items-center space-x-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {Array.from(new Set(repositories?.map((repo) => repo.language).filter(Boolean))).map((language) => (
                <DropdownMenuCheckboxItem
                  key={language}
                  checked={filters.language === language}
                  onCheckedChange={() => setFilters({ ...filters, language: language || "all" })}
                >
                  {language}
                </DropdownMenuCheckboxItem>
              ))}
              ...
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={() => setFilters({ ...filters, privacy: "all" })}
              >
                All Repositories
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.privacy === "public"}
                onCheckedChange={() => setFilters({ ...filters, privacy: "public" })}
              >
                Public Only
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.privacy === "private"}
                onCheckedChange={() => setFilters({ ...filters, privacy: "private" })}
              >
                Private Only
              </DropdownMenuCheckboxItem>
              <DropdownMenuSeparator />
              ...
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={() => setFilters({ ...filters, sortBy: "name" })}
              >
                Name
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.sortBy === "stars"}
                onCheckedChange={() => setFilters({ ...filters, sortBy: "stars" })}
              >
                Stars
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.sortBy === "forks"}
                onCheckedChange={() => setFilters({ ...filters, sortBy: "forks" })}
              >
                Forks
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.sortBy === "updated"}
                onCheckedChange={() => setFilters({ ...filters, sortBy: "updated" })}
              >
                Last Updated
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={filters.sortBy === "created"}
                onCheckedChange={() => setFilters({ ...filters, sortBy: "created" })}
              >
                Date Created
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
        ...
        {isLoadingRepos ? (
          <div className="py-12 text-center">
            <RefreshCw className="mx-auto h-8 w-8 animate-spin text-gray-400" />
            <p className="mt-4 text-gray-600 dark:text-gray-300">Loading repositories...</p>
          </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {repo.description && (
                      <p className="mt-1 text-gray-600 text-sm dark:text-gray-300">{repo.description}</p>
                    )}
                  ...
                  <div className="ml-4 flex space-x-2">
                    ...
                    <Button
                      ...
                      disabled={generateReadme.isPending || getJobStatus(repo.id) !== null}
                    >
                      {(() => {
                        ...
                        return generateReadme.isPending ? "Starting..." : "Generate README";
                      })()}
                    </Button>
                  </div>
                </div>
      ...
      <Dialog open={isReadmeDialogOpen} onOpenChange={setIsReadmeDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Configure how your README should be generated for this repository.</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end space-x-2 pt-4">
            <Button variant="outline" onClick={() => setIsReadmeDialogOpen(false)}>
              Cancel
            </Button>
