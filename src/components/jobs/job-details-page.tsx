...
import { ArrowLeft, BookOpen, CheckCircle, Clock, Copy, Download, RefreshCw, XCircle } from "lucide-react";
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h1 className="mt-4 font-bold text-2xl text-gray-900 dark:text-white">Job Not Found</h1>
        ...
        <div className="mb-8 rounded-lg bg-white p-6 shadow dark:bg-slate-800">
          <div className="flex items-start justify-between">
            ...
            {job.status === "COMPLETED" && latestVersion && (
              <div className="flex items-center space-x-2">
                <Button variant="outline" size="sm" onClick={() => copyToClipboard(latestVersion.content)}>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => downloadReadme(latestVersion.content, `${job.repository?.name || "readme"}-${job.id}`)}
                >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="lg:col-span-1">
            <div className="rounded-lg bg-white p-6 shadow dark:bg-slate-800">
              <h2 className="mb-4 font-semibold text-gray-900 text-lg dark:text-white">Job Information</h2>
              <div className="space-y-4">
                <div>
                  <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Job ID</div>
                  <p className="mt-1 font-mono text-gray-900 text-sm dark:text-white">{job.id}</p>
                </div>
                <div>
                  <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Repository</div>
                  <p className="mt-1 text-gray-900 text-sm dark:text-white">{job.repository?.name || "Unknown"}</p>
                </div>
                <div>
                  <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Style</div>
                  <p className="mt-1 text-gray-900 text-sm capitalize dark:text-white">{job.style}</p>
                </div>
                <div>
                  <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Features</div>
                ...
                {job.customPrompt && (
                  <div>
                    <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Custom Prompt</div>
                    <p className="mt-1 text-gray-900 text-sm dark:text-white">{job.customPrompt}</p>
                  </div>
                )}
                {latestVersion && (
                  <div>
                    <div className="font-medium text-gray-500 text-sm dark:text-gray-400">Content Stats</div>
                    <div className="mt-1 text-gray-900 text-sm dark:text-white">
                      ...
                      {latestVersion.modelUsed && <p>Model: {latestVersion.modelUsed}</p>}
                      {latestVersion.tokensUsed && <p>Tokens: {latestVersion.tokensUsed}</p>}
                      {latestVersion.generationTime && <p>Time: {latestVersion.generationTime}ms</p>}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-lg bg-white shadow dark:bg-slate-800">
              <div className="border-gray-200 border-b px-6 py-4 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-gray-900 text-lg dark:text-white">Generated README</h2>
                  {job.status === "PROCESSING" || job.status === "QUEUED" ? (
                    ...
                    <div className="flex items-center space-x-2">
                      <Button variant="ghost" size="sm" onClick={() => copyToClipboard(latestVersion.content)}>
                        <Copy className="mr-2 h-4 w-4" />
                        Copy
                      </Button>
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          downloadReadme(latestVersion.content, `${job.repository?.name || "readme"}-${job.id}`)
                        }
                      >
                        <Download className="mr-2 h-4 w-4" />
                        Download
                    ...
                    <div className="text-red-600 dark:text-red-400">
                      ...
                      {job.errorMessage && <p className="text-red-500 text-xs dark:text-red-400">{job.errorMessage}</p>}
                    </div>
                  ) : null}
                </div>
              </div>
              <div className="p-6">
                {job.status === "COMPLETED" && latestVersion ? (
                  ...
                  <div className="py-12 text-center">
                    <XCircle className="mx-auto h-12 w-12 text-red-500" />
                    <h3 className="mt-4 font-semibold text-gray-900 text-lg dark:text-white">Generation Failed</h3>
                    <p className="mt-2 text-gray-600 dark:text-gray-300">
                  ...
                  <div className="py-12 text-center">
                    ...
                    {job.progress !== null && (
                      <div className="mt-4">
                        ...
                        <p className="mt-2 text-gray-500 text-sm dark:text-gray-400">{job.progress}% complete</p>
                      </div>
                    )}
                  </div>
                )}
