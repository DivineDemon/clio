...
try {
  const privateKeyPath = join(process.cwd(), "src", "lib", "github-private-key.pem");
  ...
  throw new Error(`Failed to load GitHub private key: ${error instanceof Error ? error.message : "Unknown error"}`);
}
...
export async function getRepoInstallation(owner: string, repo: string) {
  try {
    ...
    logger.error("Failed to get installation", error as Error, {
      owner,
      repo,
    });
    return null;
...
export async function getInstallationToken(installationId: number) {
  try {
    ...
    logger.error("Failed to get installation token", error as Error, {
      installationId,
    });
    return null;
  }
...
export async function isAppInstalled(owner: string, repo: string): Promise<boolean> {
  const installationId = await getRepoInstallation(owner, repo);
...
export async function getInstallationUrl(owner?: string) {
  try {
    ...
    logger.error("Failed to get GitHub App info", error as Error);
    const fallbackUrl = "https://github.com/apps/clio-muse-of-history/installations/new";
...
export function verifyWebhookSignature(payload: string, signature: string): boolean {
  try {
    ...
    return timingSafeEqual(Buffer.from(expectedSignature, "hex"), Buffer.from(calculatedSignature, "hex"));
  } catch (error) {
