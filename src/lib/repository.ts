...
export async function getRepositoryInfo(owner: string, repo: string): Promise<RepositoryInfo | null> {
  try {
    ...
    logger.error("Failed to get repository info", error as Error, {
      owner,
      repo,
    });
    return null;
...
export async function getRepositoryTree(owner: string, repo: string, branch?: string): Promise<FileTreeItem[] | null> {
  try {
    const octokit = await createInstallationOctokit(owner, repo);
    ...
    logger.error("Failed to get repository tree", error as Error, {
      owner,
      repo,
    });
    return null;
...
export async function getRepositoryStructure(
  ...
  const [info, tree] = await Promise.all([getRepositoryInfo(owner, repo), getRepositoryTree(owner, repo)]);
