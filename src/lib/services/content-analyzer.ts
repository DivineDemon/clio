...
export class ContentAnalyzer {
	/**
	 * Analyze repository content for README generation
	 */
	...
	/**
	 * Build hierarchical file structure from flat tree
	 */
	...
	/**
	 * Identify key files for README generation
	 */
	private async identifyKeyFiles(
		...
		for (const item of tree) {
			if (
				item.type === "blob" &&
				importantFiles.some((file) => item.path.includes(file))
			) {
				try {
	...
	private async getFileContent(
		...
		try {
			...
			console.warn(`Error getting file content for ${path}:`, error);
			return "";
		}
	}
	...
	private async getPackageInfo(
		...
		const packageFile = keyFiles.find((file) =>
	...
	/**
	 * Check if file exists in key files
	 */
	...
	private hasDockerFiles(keyFiles: KeyFile[]): boolean {
		return (
			this.hasFile(keyFiles, "Dockerfile") ||
			this.hasFile(keyFiles, "docker-compose")
		);
	}

	/**
	 * Check if repository has CI files
	 */
	private hasCIFiles(keyFiles: KeyFile[]): boolean {
		return keyFiles.some(
			(file) =>
				file.path.includes(".github/workflows") ||
				file.path.includes(".github/actions"),
		);
	}

	/**
	 * Check if repository has license file
	 */
	private hasLicenseFile(keyFiles: KeyFile[]): boolean {
		const licensePatterns = ["LICENSE", "LICENCE", "COPYING"];
		return keyFiles.some((file) =>
			licensePatterns.some((pattern) => file.path.includes(pattern)),
		);
	}

	/**
	 * Detect framework from files and package.json
	 */
	private detectFramework(
		...
		if (keyFiles.some((f) => f.path.includes("webpack.config")))
			return "Webpack";
	...
	/**
	 * Detect build tool from files and package.json
	 */
	...
	/**
	 * Detect test framework from files and package.json
	 */
